...
def process_values(infp, keys, pols):
    ...
    antenna = find_antenna(keys[0], ['SRC/SYS'])
    if not antenna:
        print 'Antenna missing from TSYS group'
        skip_values(infp)
        return
    try:
        antenna = antenna_map[antenna]
    except:
        print 'Antenna %s not present in FITS-IDI file' % antenna
        skip_values(infp)
        return
    keys = dict(keys[0])
    spws = []
    ...
    for line in infp:
        ...
        if len(fields) > 1:
            ...
            t = tm.strptime(t, "%Yy%jd%Hh%Mm%Ss")
            secs = tm.mktime(t) + timeoff - ref
            days = secs / 86400
            source = find_source(days)
            values = fields[2:]
            tsys = {'R': [], 'L': []}
            for spw in spws:
                for pol in ['R', 'L']:
